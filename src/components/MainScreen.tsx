          ...
          {Array.from({ length: totalCells }).map((_, i) => {
            ...
            const style: React.CSSProperties = {
              ...
              background: shiftNum ? (() => {
                ...
                return isLastDay ? "#fff4c5" : "#fde047";
              })() : "#fff",
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#f9fafb" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: 12, display: "flex", justifyContent: "center", gap: 8 }}>
          {[2025, 2026, 2027].map(y => <button key={y} onClick={() => setYearlyYear(y)} style={{ padding: "4px 12px", borderRadius: 6, fontSize: 14, fontWeight: 600, border: "none", cursor: "pointer", backgroundColor: yearlyYear === y ? "#2563eb" : "#e5e7eb", color: yearlyYear === y ? "#fff" : "#374151" }}>{y}</button>)}
        </div>
        <div style={{ padding: 12, display: 'flex', justifyContent: 'center' }}>
          <button onClick={captureYearGrid} style={{ backgroundColor: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 14px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>📷 Ekran Görüntüsü Al</button>
        </div>
        ...
        <div style={{ padding: "0 16px 16px", fontSize: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 16, height: 16, border: "1px solid #111827", background: "#fde047" }} /><span>Vardiya Günü</span></div>
        </div>
      </div>
    );
  }
